...
while [ "$cnt" -gt 0 ]; do
    ...
    if [ -f "$(dirname "$dir")/operations_tmp/operation-2.json" ]; then
        ...
        if [ -n "$pid" ]; then
            ...
            # 卡少的时候，须串行处理
            export SERVE_BASE_URL="http://localhost:2026/v1"
            nohup python $dir/main_batch.py --task_id=[1,2] --task_step=2 > output2_12.log 2>&1 &
    ...
    if [ -f "$(dirname "$dir")/operations_tmp/operation-4.json" ]; then
        ...
        if [ -n "$pid" ]; then
            ...
            nohup python $dir/main_batch.py --task_id=[3,4] --task_step=2 > output2_34.log 2>&1 &
            # echo 'you can check by cmd: ps -ef | grep main_batch.py | grep -v grep'
        fi
    fi
    ...
    if [ -f "$(dirname "$dir")/operations_tmp/operation-6.json" ]; then
        ...
        if [ -n "$pid" ]; then
            ...
            nohup python $dir/main_batch.py --task_id=[5,6] --task_step=2 > output2_56.log 2>&1 &
            # echo 'you can check by cmd: ps -ef | grep main_batch.py | grep -v grep'
        fi
    fi
    ...
    if [ -f "$(dirname "$dir")/operations_tmp/operation-8.json" ]; then
        ...
        if [ -n "$pid" ]; then
            ...
            nohup python $dir/main_batch.py --task_id=[7,8] --task_step=2 > output2_78.log 2>&1 &
            # echo 'you can check by cmd: ps -ef | grep main_batch.py | grep -v grep'
        fi
    fi
