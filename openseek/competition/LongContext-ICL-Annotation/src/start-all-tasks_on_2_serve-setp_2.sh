...
if [ -n "$pid" ]; then
    ...
    nohup python $dir/main_batch.py --task_id=[1,2,3,4] --task_step=2 > output2_1234.log 2>&1 &
...
if [ -n "$pid" ]; then
    ...
    # 卡少的时候，须串行处理
    export SERVE_BASE_URL="http://localhost:2026/v1"
    nohup python $dir/main_batch.py --task_id=[5,6,7,8] --task_step=2 > output2_5678.log 2>&1 &
    # echo 'you can check by cmd: ps -ef | grep main_batch.py | grep -v grep'
fi
