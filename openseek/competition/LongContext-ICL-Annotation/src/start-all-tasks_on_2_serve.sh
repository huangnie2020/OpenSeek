...
if [ -n "$pids" ]; then
    ...
    export SERVE_BASE_URL="http://localhost:2026/v1"
    nohup python $dir/main_batch.py --task_id=[1,2,3,4] --task_step=0 > output0_1234.log 2>&1 &

    export SERVE_BASE_URL="http://localhost:2027/v1"
    nohup python $dir/main_batch.py --task_id=[5,6,7,8] --task_step=0 > output0_5678.log 2>&1 &
